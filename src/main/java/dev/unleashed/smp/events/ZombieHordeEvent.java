...
public final class ZombieHordeEvent extends AbstractEvent {
    private int taskId = -1;
    public ZombieHordeEvent() { super("zombie_horde"); }
    @Override public @NotNull String getDescription() { return "Swarms of zombies."; }
    @Override public void onStart(@NotNull EventContext ctx, @NotNull Set<Player> players) {
        final int perWave = cfgInt(ctx, "settings.zombies-per-wave", 8);
        final long interval = (long) ctx.config().events().getDouble("events.zombie_horde.settings.wave-interval", 80);
